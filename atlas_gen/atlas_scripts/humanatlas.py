...
# # """
# #     Some regions have a corresponding label in the atlas, and some don't.
# #     We will travers the tree from it's leaves to it's root, at each step
# #     for each region we will consider the part of annotated_volume
# #     corresponding to the region's or its childerns' label as the volume
# #     occupied by the region and we will extract the mesh from it
# # """
# # def filter_tree_depth(depth, node):
# #     # Keeps only tree items at a given depth
...
# # for depth in np.arange(tree.depth())[::-1]:
# #     items = list(tree.filter_nodes(partial(filter_tree_depth, depth)))
# #     print(f'Processing depth: {depth}, found {len(items)} items')

# #     for item in track(items, total=len(items), description = 'Extracing meshes'):
# #         savepath = meshes_dir_path/ f'{item.identifier}.obj'
# #         if savepath.exists(): continue

# #         stree = tree.subtree(item.identifier)
# #         ids = list(stree.nodes.keys())
# #         matched_labels = [i for i in ids if i in labels]
# #         if not matched_labels : # it fails if the region and all of it's children are not in annotation
# #             print(f'No labels found for {item.tag}')
# #             continue
# #         else:
...
# #             if not np.max(mask):
# #                 print(f'Empty mask for {item.tag}')
# #             else:
# #                 extract_mesh_from_mask(mask, obj_filepath=savepath)
...
#     structures, annotation_whole, meshes_dir, acronym_to_voxel = args

#     lbl = structures.loc[acronym_to_voxel, "acronym"]
#     volume = volume_utils.create_masked_array(
#         annotation_whole, acronym_to_voxel, greater_than=False
#     )
...
#         return None
#     else:
#         print(f"Creating mesh for {acronym_to_voxel}.obj")
#         savepath = meshes_dir / f"{acronym_to_voxel}.obj"
