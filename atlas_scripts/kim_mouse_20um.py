...
class AtlasBuilder:
    """Class to handle the download, retrieval, and processing of a brain atlas.

    This class provides methods to:
        - Download necessary atlas resources (reference, annotation, ontology).
        - Load reference and annotation volumes as numpy arrays.
        - Extract structure information from the atlas ontology.
        - Construct 3D meshes for atlas structures.

    Attributes
    ----------
    bg_root_dir : Path
        Root working directory for atlas files.
    download_folder : Path
        Directory where downloaded resources are stored.
    download_dir_content : set
        Set of filenames present in the download folder.
    reference : np.ndarray
        The reference volume of the atlas.
    annotation : np.ndarray
        The annotation volume of the atlas.
    structures : list of dict
        List of structure information dictionaries, including id, name,
        acronym, path to root, and RGB triplet for visualization.

    Methods
    -------
    download_resources()
        Downloads atlas files and saves them in the download folder.
    retrieve_reference_and_annotation() -> Tuple[np.ndarray, np.ndarray]
        Loads the reference and annotation volumes as numpy arrays.
    retrieve_hemisphere_map() -> None
        A placeholder method present in original script.
    retrieve_structure_information() -> List[Dict[str, Any]]
        Returns a list of dictionaries with information about each atlas structure.
    ...
    def download_resources(self) -> None:
        """Downloads the necessary resources for the atlas.
        ...
        if (
            ONTOLOGY_FILE not in initial_download_folder_content
    ...
    def retrieve_reference_and_annotation(
        ...
        """Retrieves the desired reference and annotation as two numpy arrays.
            ...
            structures.append(
                {
                    ...
                    "name": row["name"],
                    "acronym": row["acronym"],
                    "structure_id_path": AtlasBuilder.get_path_to_root_id(
                        id_to_row, df, row["id"]
                    ),
    ...
    def get_path_to_root_id(
        id_to_row: Dict, df: pd.DataFrame, current_id: int
    ) -> List:
        ...
        df : pd.DataFrame
            DataFrame containing atlas structure information (used for reference).
        current_id : int
    ...
    def load_as_asr(path: Path) -> np.ndarray:
        """Load an imaging file and reorder it to match the ASR (Anterior-Superior-Right) convention.

        The function loads a NIfTI file using `nibabel`, converts it to a numpy array,
        squeezes singleton dimensions, flips and rotates axes to match the ASR orientation.

        Parameters
        ----------
        path : Path
            Path to the NIfTI file to be loaded.
...
if __name__ == "__main__":
    ...
    reference_volume, annotated_volume = (
        atlas_builder.retrieve_reference_and_annotation()
    )
