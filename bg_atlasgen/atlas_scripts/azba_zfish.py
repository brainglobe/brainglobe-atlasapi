...
def create_atlas(working_dir, resolution):
    ...
    for j in range(0, len(hierarchy)):
        hierarchy[j]['structure_id_path'] = list(map(int, hierarchy[j]['structure_id_path'].split("/")))
    for k in range(0, len(hierarchy)):    
        try:
            hierarchy[k]['rgb_triplet'] = list(map(int,hierarchy[k]['rgb_triplet'].split("/")))
        except ValueError:
    ...
    print(f"Number of brain regions: {tree.size()}, max tree depth: {tree.depth()}")
    ...
    print("Finished mesh extraction in : ", round((time.time() - start) / 60, 2), " minutes")    
