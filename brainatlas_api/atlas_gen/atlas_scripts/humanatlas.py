...
anatomy_whole = np.zeros(
    (anatomy.shape[0] * 2, anatomy.shape[1], anatomy.shape[2]), anatomy.dtype
)
...
query_url = (
    "http://api.brain-map.org/api/v2/data/query.json?criteria=model::Structure"
)
...
unique_values, unique_counts = np.unique(annotation_whole, return_counts=True)
