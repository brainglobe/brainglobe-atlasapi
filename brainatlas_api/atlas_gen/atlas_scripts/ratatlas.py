...
from vtkplotter import write, show, Volume
...
from atlas_gen.volume_utils import extract_volume_surface, extract_label_mesh, load_labelled_volume
...
temp_path = Path(r'D:\Dropbox (UCL - SWC)\Rotation_vte\Anatomy\Atlases\atlasesforbrainrender\goldcustomrat')
...
base_data_fld = Path(r'D:\Dropbox (UCL - SWC)\Rotation_vte\Anatomy\Atlases\atlasesforbrainrender\goldcustomrat')

for name in ['reference', 'annotated']:
    loaded = load_any(str(base_data_fld/ f'{name}.tif')) # shape (186, 160, 160)
...
hierarchy = pd.read_excel(str(base_data_fld / 'SwansonAtlasCategories-Mar_2_2005.xls'), header=1,
            usecols = ['Abbreviation', 'Name of Area', 'Major',	'Sub_Major', 'Minor'], nrows=1276)

majors = pd.read_excel(str(base_data_fld / 'SwansonAtlasCategories-Mar_2_2005.xls'), header=3,
            usecols = [13, 14], nrows=20)

submajors = pd.read_excel(str(base_data_fld / 'SwansonAtlasCategories-Mar_2_2005.xls'), header=3,
            usecols =  [15, 16], nrows=89)


clean_hierarchy = dict(abbreviation = [], name=[], major=[], majornum=[], submajor=[], submajornum=[], minor=[])
for i, region in hierarchy.iterrows():
    ...
    clean_hierarchy['major'].append(majors.loc[majors.ANC == region.Major]['ANC Name'].values[0])
    clean_hierarchy['majornum'].append(majors.loc[majors.ANC == region.Major]['ANC'].values[0])
    clean_hierarchy['minor'].append(region['Minor'])
    try:
        clean_hierarchy['submajor'].append(submajors.loc[submajors.SubANC == region.Sub_Major]['SubANC Name'].values[0])
        clean_hierarchy['submajornum'].append(int(submajors.loc[submajors.SubANC == region.Sub_Major]['SubANC'].values[0]))
    except:
...
for i, major in majors.iterrows():
    if  not isinstance(major['ANC Name'] , str): continue
...
for i, submajor in submajors.iterrows():
    ...
    try:
        entry = clean_hierarchy.loc[clean_hierarchy.submajornum == submajor['SubANC']].iloc[0]
    except:
        pass
...
for i, region in tqdm(clean_hierarchy.iterrows()):
    structure = [s for s in structures if s['acronym'] == region['abbreviation']][0]
    savepath = str(meshes_dir_path / f'{structure["id"]}.obj'.replace("/", "-").replace("\\", "-"))
...
for i, submajor in tqdm(submajors.iterrows()):
    structure = [s for s in structures if s['acronym'] == submajor['SubANC Name'].replace(" ", "-")][0]
    savepath = str(meshes_dir_path / f'{structure["id"]}.obj'.replace(" ", "-").replace("/", "-").replace("\\", "-"))
    if os.path.isfile(savepath): continue

    regions = list(clean_hierarchy.loc[clean_hierarchy.submajor == submajor['SubANC Name']].minor.values)
...
for i, major in tqdm(majors.iterrows()):
    if not isinstance(major['ANC Name'], str): continue
    structure = [s for s in structures if s['acronym'] == major['ANC Name'].replace(" ", "-")][0]
    savepath = str(meshes_dir_path / f'{structure["id"]}.obj'.replace(" ", "-").replace("/", "-").replace("\\", "-"))
    if os.path.isfile(savepath): continue

    regions = list(clean_hierarchy.loc[clean_hierarchy.major == major['ANC Name']].minor.values)
    if not regions: continue
