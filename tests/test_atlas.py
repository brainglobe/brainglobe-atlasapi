...
def test_initialization(atlas):
    assert atlas.metadata == {"name": "test",
...
@pytest.mark.parametrize("attribute, val", [("shape", [132, 80, 114]),
                                            ("resolution", [100, 100, 100]),
                                            ("name", "test"),
                                            ("symmetric", True)])
...
@pytest.mark.parametrize("stack_name, val", [("reference", [[[146, 155],
                                                             [153, 157]],
                                                            [[148, 150],
                                                             [153, 153]]]),
                                             ("annotated", [[[59, 362],
...
@pytest.mark.parametrize("coords", [[39.,  36.,  57.],
                                    (39,  36,  57),
                                    np.array([39.,  36.,  57.])])
...
def test_meshfile_from_id(atlas):
    assert atlas.get_mesh_file_from_name("CH") == atlas.root_dir / "meshes/567.obj"
